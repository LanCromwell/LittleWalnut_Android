...
public class LWheelView extends View {
    ...
    private LMessageHandler handler;
    ...
    private void initLoopView(Context context) {
        ...
        handler = new LMessageHandler(this);
        gestureDetector = new GestureDetector(context, new LLoopViewGestureListener(this));
    ...
    public void smoothScroll(LWheelView.ACTION action) {//平滑滚动的实现
        cancelFuture();
        if (action == LWheelView.ACTION.FLING || action == LWheelView.ACTION.DAGGLE) {
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new LSmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    public final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new LInertiaTimerTask(this, velocityY), 0, VELOCITY_FLING, TimeUnit.MILLISECONDS);
    }
            ...
            default:
                if (!eventConsumed) {//未消费掉事件
                    ...
                    if ((System.currentTimeMillis() - startTime) > 120) {
                        // 处理拖拽事件
                        smoothScroll(LWheelView.ACTION.DAGGLE);
                    } else {
                        // 处理条目点击事件
                        smoothScroll(LWheelView.ACTION.CLICK);
                    }
                }
                break;
        }
